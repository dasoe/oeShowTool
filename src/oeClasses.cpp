...
void oeSlot::init(int contentWidthTemp, int contentHeightTemp) {
...
ofFbo oeSlot::getContent() {
    ...
    if (targetAlpha < 255) {
        if (targetType == 0) {
            ...
            ofLogNotice("dawing targetVideo? type: " + ofToString(type) + ", targetType: " + ofToString(targetType) + ", targetAlpha: " + ofToString(targetAlpha));
